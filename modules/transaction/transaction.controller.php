...
function transaction_overview($row_id = NULL, $search = NULL, $sort = NULL, $page = 1) {
  ...
  if (!empty($search)) {
      ...
      $sql = "select SQL_CALC_FOUND_ROWS  t.id, t.type_id, tt.name as `purpose`, u.first_name, u.last_name, t.amount_value, so.description as amount_type, t.amount_identification, t.date_created, t.date_lastchange, b.bank_name, b.bank_account_number, t.comment  FROM church_finance.transaction t
                   left outer join transaction_type tt on tt.type_id = t.subtype_id
  ...
  if (count($records)) {
    ...
    $headers[] = array('display' => 'Purpose', 'field' => 'purpose');
    ...
    foreach($records as $transaction ){
      ...
      $rows[$j]['purpose']               = $transaction['purpose'];
...
function transaction_thithes($row_id = NULL, $search = NULL, $sort = NULL, $page = 1) {
  $view = new ListView();
  // Row Id for update only row
  if (!empty($row_id)) {
    $row_id = 't.id = ' . $row_id;
  ...
  if (!empty($search)) {
      ...
      $sql = "select SQL_CALC_FOUND_ROWS  t.id, t.type_id, tt.name as `purpose`, u.first_name, u.last_name, t.amount_value, so.description as amount_type, t.amount_identification, t.date_created, t.date_lastchange, b.bank_name, b.bank_account_number, t.comment  FROM church_finance.transaction t
                   ...
                   left outer join user u on u.id = t.user_id
                   left outer join select_option so on so.upstream_name = 'payment_type' and so.value = t.amount_type
              where t.church_id = ".$_SESSION['log_church_id']." and tt.id = 1 and ".$search_query." order by ".$sort." limit ".$limit." offset ".$offset;
      $records = $pdo->prepare($sql);
      $records->execute();
  } else {
      $sql = "select SQL_CALC_FOUND_ROWS t.id, t.type_id, tt.name as `purpose`, u.first_name, u.last_name, t.amount_value, so.description as amount_type, t.amount_identification, t.date_created, t.date_lastchange, b.bank_name, b.bank_account_number, t.comment  FROM church_finance.transaction t
                   ...
                   left outer join user u on u.id = t.user_id
                   left outer join select_option so on so.upstream_name = 'payment_type' and so.value = t.amount_type
              where t.church_id = ".$_SESSION['log_church_id']."  and tt.id = 1 order by ".$sort." limit ".$limit." offset ".$offset;
      $records = $pdo->prepare($sql);
      $records->execute();
  }
  $total_records = $pdo->query('SELECT FOUND_ROWS();')->fetch(PDO::FETCH_COLUMN);
  ...
  if (count($records)) {
    ...
    $headers[] = array('display' => 'Amount Type', 'field' => 'amount_type');
    $headers[] = array('display' => 'Amount', 'field' => 'amount_value');
    $headers = build_sort_header('transaction_thithes', 'transaction', $headers, $sort);
  ...
  $options = array(
    ...
    'table_prefix' => theme_link_process_information(translate('Create New Transaction'),
      ...
      'transaction',
      array('response_type' => 'modal')),
    'pager_items' => build_pager('transaction_thithes', 'transaction', $total_records, $limit, $page),
    'page' => $page,
    'sort' => $sort,
    'search' => $search,
    'show_search' => TRUE,
    'function' => 'transaction_thithes',
    'module' => 'transaction',
    'update_row_id' => '',
    'table_form_id' => '',
    'table_form_process' => '',
...
function transaction_offers($row_id = NULL, $search = NULL, $sort = NULL, $page = 1) {
  $view = new ListView();
  // Row Id for update only row
  if (!empty($row_id)) {
    $row_id = 't.id = ' . $row_id;
  ...
  if (!empty($search)) {
      ...
      $sql = "select SQL_CALC_FOUND_ROWS  t.id, t.type_id, tt.name as `purpose`, u.first_name, u.last_name, t.amount_value, so.description as amount_type, t.amount_identification, t.date_created, t.date_lastchange, b.bank_name, b.bank_account_number, t.comment  FROM church_finance.transaction t
                   ...
                   left outer join user u on u.id = t.user_id
                   left outer join select_option so on so.upstream_name = 'payment_type' and so.value = t.amount_type
              where t.church_id = ".$_SESSION['log_church_id']." and tt.id = 2 and ".$search_query." order by ".$sort." limit ".$limit." offset ".$offset;
      $records = $pdo->prepare($sql);
      $records->execute();
  } else {
      $sql = "select SQL_CALC_FOUND_ROWS t.id, t.type_id, tt.name as `purpose`, u.first_name, u.last_name, t.amount_value, so.description as amount_type, t.amount_identification, t.date_created, t.date_lastchange, b.bank_name, b.bank_account_number, t.comment  FROM church_finance.transaction t
                   ...
                   left outer join user u on u.id = t.user_id
                   left outer join select_option so on so.upstream_name = 'payment_type' and so.value = t.amount_type
              where t.church_id = ".$_SESSION['log_church_id']."  and tt.id = 2 order by ".$sort." limit ".$limit." offset ".$offset;
      $records = $pdo->prepare($sql);
      $records->execute();
  }
  $total_records = $pdo->query('SELECT FOUND_ROWS();')->fetch(PDO::FETCH_COLUMN);
  ...
  if (count($records)) {
    ...
    $headers[] = array('display' => 'Amount Type', 'field' => 'amount_type');
    $headers[] = array('display' => 'Amount', 'field' => 'amount_value');
    $headers = build_sort_header('transaction_offers', 'transaction', $headers, $sort);
  ...
  $options = array(
    ...
    'table_prefix' => theme_link_process_information(translate('Create New Transaction'),
      ...
      'transaction',
      array('response_type' => 'modal')),
    'pager_items' => build_pager('transaction_offers', 'transaction', $total_records, $limit, $page),
    'page' => $page,
    'sort' => $sort,
    'search' => $search,
    'show_search' => TRUE,
    'function' => 'transaction_offers',
    'module' => 'transaction',
    'update_row_id' => '',
    'table_form_id' => '',
    'table_form_process' => '',

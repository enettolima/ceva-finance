    ...
    for($i=0; $i<$menus->affected; $i++){
      ...
      switch($each->allow)
      {
        ...
        case "between":
          ...
          if($range[0]<$level && $level<$range[1]){ $build=true; }else{ $build=false; }
          break;
        case "equal":
          if($each->allow_value == $level){ $build=true; }else{ $build=false;}
          break;
        case "higher":
          if($each->allow_value<$level){ $build=true; }else{ $build=false;}
          break;
...
function build_login_submenu($level)
{
  ...
  if($menus->affected)
  {
    ...
    for($i=0; $i<$menus->affected; $i++){
      ...
      if($each->initial){ $class="active"; $build=true; $content=execute_initial_function($each->func, $level);}
...
function build_login_sidemenu($level) {
  ...
  if($menus->affected) {
    ...
    for($i=0; $i<$menus->affected; $i++){
      ...
      if($each->initial){ $class="active"; $build=true; }
    ...
    case 1:
      ...
      if($main->affected)
      {
        ...
        for($i=0; $i<$main->affected; $i++){
          ...
          if($each->element_name==$data['clicked']){ 
            $class="active"; $build=true; $main_selected_id=$each->id;
          }
          if($build && $each->status==1)
          {
      ...
      if($sub->affected)
      {
        ...
        for($i=0; $i<$sub->affected; $i++){
          ...
          if($each->position==0){ $class="active"; $build=true; $sub_selected_id=$each->id; }
      ...
      if($side->affected)
      {
        ...
        for($i=0; $i<$side->affected; $i++){
          ...
          if($each->position==0){ $class="active"; $build=true; }
    ...
    case 2:
      ...
      if($sub->affected)
      {
        ...
        for($i=0; $i<$sub->affected; $i++){
          ...
          if($each->element_name==$data['clicked']){ $class="active"; $build=true; $sub_selected_id=$each->id; }
    ...
    case 3:
      ...
      if($menus->affected)
      {
        ...
        for($i=0; $i<$menus->affected; $i++){
          ...
          if($each->element_name==$data['clicked']){ $class="active"; $build=true;}

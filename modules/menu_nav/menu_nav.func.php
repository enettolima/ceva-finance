...
function menu_constructor($level, $dash_show = 0) {
  ...
  $menu = '';
  $default_menu_item_class = 'fa fa-angle-double-right';
  if ($main_menu->affected) {
    ...
    foreach ($main_menu->data as $main_key => $main_item) {
      ...
      if (menu_permission($main_item, $level)) {
        ...
        if ($sub_menu->affected) {
          foreach ($sub_menu->data as $sub_item) {
            if (menu_permission($sub_item, $level)) {
              ...
              if ($side_menu->affected) {
                ...
                foreach ($side_menu->data as $side_item) {
                  if (menu_permission($side_item, $level)) {
                    ...
                    $icon_class = ($side_item['icon_class']) ? $side_item['icon_class'] : $default_menu_item_class;
                    $menuside .= '<li id="' . $side_item['element_name'] . '"> <a href="' . $href . '" > <i class="' . $icon_class . '"></i> ' . translate($side_item['label'], $_SESSION['log_preferred_language']) . '</a> </li>';
                  }
                }
                if ($menuside) {
                  $treeview = 'treeview';
              ...
              // Adding specific icon.
              $icon_class = ($sub_item['icon_class']) ? $sub_item['icon_class'] : $default_menu_item_class;
              $menusub .= '<li class="' . $treeview . '" id="' . $sub_item['element_name'] . '"><a href="' . $href . '" > <i class="' . $icon_class . '"></i> ' . translate($sub_item['label'], $_SESSION['log_preferred_language']) . $treeview_arrow . '</a>' . $menuside . '</li>';
            }
          }
          if ($menusub) {
            $treeview = 'treeview';
        ...
        // Adding specific icon.
        $icon_class = ($main_item['icon_class']) ? $main_item['icon_class'] : $default_menu_item_class;
        $menu .= '<li id="' . $main_item['element_name'] . '" class="' . $active . ' ' . $treeview . ' ' . $main_item_class . '"> <a href="' . $href . '" > <i class="' . $icon_class . '"></i>' . translate($main_item['label'], $_SESSION['log_preferred_language']) . $treeview_arrow . '</a>' . $menusub . '</li>';
      }
    }
    $menu .= '</ul>';
  }

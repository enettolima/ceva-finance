    ...
    $.ajax({
      ...
      success: function(transport) {
        ...
        if (resp[0] == 'ERROR' || resp[0] == 'ALERT') {
          // ERROR MESSAGES WILL BE HANDLED SEPARATELY
          ...
          switch (response_type) {
            case 'modal':
              ...
              break;
            case 'update_row':
              // Need to get pager information otherwise it will go back to first page after row update
              // var form_action = $('.form.table-info').attr('action');
              // $('.form.table-info').attr('action', form_action.replace('_search', '_pager')).submit();
              obj = JSON.parse(transport);
              var row = $('[data-info-id="' + obj.id + '"]');
              $(row).children('td').each(function() {
                if ($(this).data('info-field') in obj) {
                  $(this).html(obj[$(this).data('info-field')]).fadeOut('fast').fadeIn('slow');
                }
              });
              break;
            case 'append':
...
function file_delete(file, e) {
    process_information(null, 'file_remove', 'upload', null, 'file|' + file, null, 'file-message');
    $(e).parent().remove();
    var files = '';

...
$('#menu-hide-button').click(function() {
  ...
  process_information(null, 'update_user_dash_model', 'user_group', null, 'setdash|' + set_dash, null, 'status-message');
})
...
function dashboard_action() {
    $('#sortable1, #sortable2, #sortable3').sortable({
        ...
        update: function(event, ui) {
            ...
            process_information('dashboard-form', 'dashboard_update_list', 'dashboard', null, 'positions|' + positions1 + '-' + positions2 + '-' + positions3, null, 'status-message', null);
        }
    });
...
function dashboard_setup() {
    process_information('dashboard-setup-form', 'dashboard_setup', 'dashboard', null, null, null, 'dashboard-widgets');
}
...
function dashboard_delete_widget(widget) {
    $('#' + widget).fadeIn('fast', function() {
        ...
        /*var i = 0;
         ...
         process_information('dashboard-form', 'dashboard_update_list', 'dashboard', null, 'positions|'+positions, null, 'status-message', null);*/
        process_information('dashboard-setup-form', 'dashboard_setup', 'dashboard', null, null, null, 'dashboard-widgets');
    });
}
 ...
 */
function file_uploader(field_id) {
  var preview = '';
  var button = $('#upload-button-' + field_id), interval;
  new AjaxUpload(button, {
    action: 'modules/upload/uploader.php?field_id=' + field_id,
    name: 'myfile',
      ...
      console.debug(response);
        if (response.uploaded == true) {
          ...
          if (response.preview == true) {
            preview = '<img src="' + response.preview_uri + '" class="img-thumbnail" /><br/>';
          }
          $('<li class="file-item"></li>').appendTo('#uploaded-files-' + field_id).html(preview + '<span class="file-name">' + file + '</span> <a class="file-delete" title="Delete" onClick="file_delete(\'' + file + '\', this, ' + field_id +');"><i class="fa fa-trash-o"></i></a>');
          $('#files-' + field_id).val($('#files-' + field_id).val() + file + '\n');
 ...
 */
function file_delete(file, clicked_el, field_id) {
  process_information(null, 'file_remove', 'upload', null, 'file|' + file, null, '', 'in_modal');
  $(clicked_el).parent().remove();
  var files = '';
  $('#uploaded-files-' + field_id + ' .file-name').each(function() {
    files += $(this).text() + '\n';
  });
  $('#files-' + field_id).val(files);
  $('#upload-button-' + field_id).fadeIn();
}
...
function callmetorec_auto_refresh(mediaID, filename, callback) {
    $('.calltorecord').dialog({
        ...
        close: function() {
            ...
            process_information(null, 'audio_prompt_list', 'media');
            $(this).remove();
        }
    });
